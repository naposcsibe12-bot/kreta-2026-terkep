import { POIS, baseForDay, haversineKm, type Poi, type Category, type Base } from './data';
...
export function loadLocks():Record<string,string[]> { try{return JSON.parse(localStorage.getItem(LOCK)||'{}') as Record<string,string[]>}catch{return{}} }
...
function score(p:Poi,day:number,prefs:Preferences,existing:string[]):number{
 const base:Base=baseForDay(day); const d=haversineKm(base,p); let s=Math.max(0,80-d*1.15);
...
export function recommend(day:number,prefs:Preferences=defaultPrefs):Poi[]{
 const plan=loadPlan(), existing=plan[`D${day}`]||[]; const selected:Poi[]=[]; const counts:Partial<Record<Category,number>>={};
 [...POIS].filter(p=>!existing.includes(p.id)).sort((a,b)=>score(b,day,prefs,existing)-score(a,day,prefs,existing)).forEach(p=>{ if(selected.length>=5)return; const c=counts[p.cat]||0; if(c>=2)return; selected.push(p); counts[p.cat]=c+1; });
 return selected;
}
...
export function buildDay(day:number,prefs:Preferences=defaultPrefs):Poi[]{
 ...
 const out:Poi[]=[...locked]; const usedCats=new Set<Category>(out.map(p=>p.cat));
...
export function dayPois(day:number):Poi[]{ const plan=loadPlan(); return (plan[`D${day}`]||[]).map(id=>POIS.find(p=>p.id===id)).filter((p):p is Poi=>Boolean(p)); }
export function routeOrder(day:number):Poi[]{
 const points=dayPois(day); const base:Base=baseForDay(day); const locks=loadLocks()[`D${day}`]||[]; if(!points.length)return[];
 const ordered:Poi[]=[]; let current:{lat:number;lon:number}=base; const remaining=points.filter(p=>!locks.includes(p.id));
 while(remaining.length){ let best=0,bd=Infinity; remaining.forEach((p,i)=>{const d=haversineKm(current,p);if(d<bd){bd=d;best=i;}}); const next=remaining.splice(best,1)[0]; ordered.push(next); current={lat:next.lat,lon:next.lon}; }
 return [...points.filter(p=>locks.includes(p.id)),...ordered];
}
